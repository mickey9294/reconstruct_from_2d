...
%function [bestnest,fmin]=CS(n)
function [fbestval,bestmember,time,GlobalMinIters,N_iter] = CS(objfun,NP,maxCycle,D,Lb0,Ub0,runtime)
%if nargin<1,
...
GlobalMinIters=repmat(1.0e+9,runtime,maxCycle);
N_iter=zeros(runtime,1);
%%%%%%%%%%%%%%%%%%%%%%
for run=1:runtime
    ...
    [fmin,bestnest,nest,fitness]=get_best_nest(objfun,nest,nest,fitness);
    GlobalParams=bestnest;
    N_iter(run)=0;
    %% Starting iterations
    %while (fmin>Tol),
    for t=1:maxCycle
        ...
        [fnew,best,nest,fitness]=get_best_nest(objfun,nest,new_nest,fitness);
        % Update the counter
        N_iter(run)=N_iter(run)+n;
...
function [fmin,best,nest,fitness]=get_best_nest(objfun,nest,newnest,fitness)
% Evaluating all new solutions
for j=1:size(nest,1),
    ObjVal=feval(objfun,newnest(j,:));
    fnew=ObjVal;
    if fnew<=fitness(j),
        fitness(j)=fnew;
        nest(j,:)=newnest(j,:);
